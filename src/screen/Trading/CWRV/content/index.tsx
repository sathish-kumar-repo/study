import Part1 from "../code/Part 1";
import Part10 from "../code/Part 10";
import Part11 from "../code/Part 11";
import Part12 from "../code/Part 12";
import Part13 from "../code/Part 13";
import Part14 from "../code/Part 14";
import Part15 from "../code/Part 15";
import Part2 from "../code/Part 2";
import Part3 from "../code/Part 3";
import Part4 from "../code/Part 4";
import Part5 from "../code/Part 5";
import Part6 from "../code/Part 6";
import Part7 from "../code/Part 7";
import Part8 from "../code/Part 8";
import Part9 from "../code/Part 9";

const cwrvContent = {
  about: {
    subCategory: "Binary Options",
    name: "Binary Options (CWRV)",
    description: "Learn about Candlestick Wick Reading with Volume (CWRV)",
    link: "",
    img: "CWRV.jpg",
  },
  route: [
    { topic: "Part 1", page: <Part1 /> },
    { topic: "Part 2", page: <Part2 /> },
    { topic: "Part 3", page: <Part3 /> },
    { topic: "Part 4", page: <Part4 /> },
    { topic: "Part 5", page: <Part5 /> },
    { topic: "Part 6", page: <Part6 /> },
    { topic: "Part 7", page: <Part7 /> },
    { topic: "Part 8", page: <Part8 /> },
    { topic: "Part 9", page: <Part9 /> },
    { topic: "Part 10", page: <Part10 /> },
    { topic: "Part 11", page: <Part11 /> },
    { topic: "Part 12", page: <Part12 /> },
    { topic: "Part 13", page: <Part13 /> },
    { topic: "Part 14", page: <Part14 /> },
    { topic: "Part 15", page: <Part15 /> },
  ],
};

export default cwrvContent;
