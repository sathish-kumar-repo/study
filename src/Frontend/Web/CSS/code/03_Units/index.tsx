...
const Units = () => (
  <>
    ...
    <Para>
      Every property like margin, padding, height, width, font-size, etc in CSS
      that has some dimensions or length needs a unit. CSS provides us with lots
      of units, some of whose values are fixed and are called absolute units
      while there are others whose values are relative to other values like that
      of the parent element’s or to the default value for that particular HTML
      element, these are called relative units.
    </Para>

    <H1>Absolute units</H1>
    <List
      items={["centimeter", "millimeter", "inches", "pixel", "point", "picas"]}
    />
    <Para>
      These units are the ones whose values are fixed irrespective of any other
      factors like parent element or viewing window i.e the screen size won't
      affect the size of the element.
    </Para>
    ...
    <H2>CSS - pt</H2>
    <Para>pt stands for point. 1 CSS pt is defined as 1/72th of an inch.</Para>
    <Para>
      This unit is mainly used in printers for printing the output on paper and
      not so widely used for on-screen outputs.
    </Para>
    <H2>CSS - pc</H2>
    <Para>
    ...
    <Para>
      In relative units, we talk in terms of the same property, like, if we are
      talking about width of an element in relative units then it is relative to
      the WIDTH of the parent element/viewport.
    </Para>
    <Para>
      Relative units, if used correctly, are suitable for making elements scale
      properly with respect to other things on the same page
    </Para>

    <H2>CSS - % (percentage)</H2>
    <Para>
      Percentage is widely used for making responsive sites. This allows us to
      size HTML elements dynamically relative to the size of the viewing window.
    </Para>
    <H2>CSS - em</H2>
    <Para>
    ...
    <Para>
      This is mostly used to achieve the same values dynamically as is the case
      with % but applicable specifically to font size.
    </Para>
    <H2>CSS - rem</H2>
    <Para>
      This unit counters the adding-up effect of units like % and em. rem rather
      stands for Root em.
    </Para>
    <Para>
    ...
    <H2>CSS - vw</H2>
    <Para>
      vw stands for View Width. 100vw means 100% the width of the viewport/view
      window.
    </Para>
    <Para>
